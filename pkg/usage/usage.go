...
func CustomUsage() {
	fmt.Fprintf(os.Stderr, "MAYHEM(1)                    Chaos Engineering Tool                    MAYHEM(1)\n\n")

	fmt.Fprintf(os.Stderr, "NAME\n")
	fmt.Fprintf(os.Stderr, "       mayhem - chaos engineering proxy for testing service resilience\n\n")

	fmt.Fprintf(os.Stderr, "SYNOPSIS\n")
	fmt.Fprintf(os.Stderr, "       mayhem -target=URL [OPTIONS]\n\n")

	fmt.Fprintf(os.Stderr, "DESCRIPTION\n")
	fmt.Fprintf(os.Stderr, "       Mayhem is a chaos engineering tool that acts as a proxy between clients\n")
	...
	fmt.Fprintf(os.Stderr, "           # Terminal 2: Start Mayhem\n")
	fmt.Fprintf(os.Stderr, "           mayhem -target=http://localhost:3000 -delay-prob=0.3 -error-prob=0.1\n\n")
	fmt.Fprintf(os.Stderr, "           # Terminal 3: Send test requests through Mayhem\n")
	fmt.Fprintf(os.Stderr, "           curl http://localhost:8080/api/users\n\n")

	fmt.Fprintf(os.Stderr, "       Load testing with chaos:\n")
	fmt.Fprintf(os.Stderr, "           # Start Mayhem with moderate chaos\n")
	fmt.Fprintf(os.Stderr, "           mayhem -target=http://localhost:3000 -delay-prob=0.2 -error-prob=0.05\n\n")
	fmt.Fprintf(os.Stderr, "           # Run load test through Mayhem\n")
	...
	fmt.Fprintf(os.Stderr, "           mayhem -target=http://localhost:3000 \\\n")
	...
	fmt.Fprintf(os.Stderr, "           mayhem -target=http://localhost:3000 -delay-prob=0.1 -error-prob=0.02\n\n")
	...
	fmt.Fprintf(os.Stderr, "           mayhem -config=chaos-config.json\n\n")

	fmt.Fprintf(os.Stderr, "OPTIONS\n")
	flag.PrintDefaults()

	fmt.Fprintf(os.Stderr, "\nCHAOS MANAGEMENT ENDPOINTS\n")
	fmt.Fprintf(os.Stderr, "       Mayhem provides HTTP endpoints for runtime management:\n\n")
	...
	fmt.Fprintf(os.Stderr, "       Mayhem won't start:\n")
	...
	fmt.Fprintf(os.Stderr, "       ~/.mayhem/config.json\n")
	...
	fmt.Fprintf(os.Stderr, "       Report bugs at: https://github.com/pgaijin66/mayhem/issues\n\n")

	fmt.Fprintf(os.Stderr, "SEE ALSO\n")
	fmt.Fprintf(os.Stderr, "       curl(1), hey(1), wrk(1)\n")
	fmt.Fprintf(os.Stderr, "       Online documentation: https://github.com/pgaijin66/mayhem\n\n")

	fmt.Fprintf(os.Stderr, "Mayhem 1.0.0                      2024                           MAYHEM(1)\n")
}
